defmodule Explorer.Chain.Cache.ThirtyDayContractCountList do
  @moduledoc """
  Cache for Thirty Day Contract Count List.
  """

  @default_cache_period :timer.hours(2)

  use Explorer.Chain.MapCache,
    name: :thirty_day_contract_count_list,
    key: :cached_result,
    key: :async_task,
    ...
    {:ok, task} =
      Task.start(fn ->
        try do
          ...
          e ->
            Logger.debug([
              "Coudn't update Thirty Day Contract Count List test #{inspect(e)}"
            ])
        end
  ...
  defp cache_period do
    "CACHE_THIRTY_DAY_CONTRACT_COUNT_LIST"
    |> System.get_env("")
    |> Integer.parse()
    |> case do
