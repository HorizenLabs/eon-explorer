    ...
    test "can import range with all synchronous imported schemas", %{
        ...
        case Keyword.fetch!(json_rpc_named_arguments, :variant) do
          EthereumJSONRPC.Nethermind ->
            ...
            |> expect(:json_rpc, fn json, _options ->
              assert [%{id: id, method: "eth_getBlockByNumber", params: [^block_quantity, true]}] =
                       json
                   ...
                   result: %{
                     ...
                     "logs" => [
                       %{
                         ...
                         "logIndex" => "0x0",
                         "topics" => [
                           "0x600bcf04a13e752d1e3670a5a9f1c21177ca2a93c6f5391d4f1298d098097c22"
                         ],
                         "transactionHash" =>
                           "0x53bd884872de3e488692881baeec262e7b95234d3965248c39fe992fffd433e5",
                         "transactionIndex" => "0x0",
            ...
            |> expect(:json_rpc, fn [%{id: id, method: "trace_block", params: [^block_quantity]}],
                                    _options ->
              {:ok, [%{id: id, result: []}]}
            end)
            |> expect(:json_rpc, fn json, _options ->
            ...
            |> expect(:json_rpc, 11, fn json, _options ->
              ...
              case request do
                ...
                %{
                  id: id,
                  method: "eth_getBalance",
                  params: [^forward_transfer_address_hash, ^block_quantity]
                } ->
                  {:ok, [%{id: id, jsonrpc: "2.0", result: "0x1"}]}
      ...
      case Keyword.fetch!(json_rpc_named_arguments, :variant) do
        EthereumJSONRPC.Geth ->
                  ...
                  %{
                    addresses: [
                      %Explorer.Chain.Hash{
                        byte_count: 20,
                        bytes:
                          <<55, 52, 203, 24, 116, 145, 237, 231, 19, 174, 91, 59, 45, 18, 40, 74,
                            244, 107, 129, 1>>
                      } = first_address_hash,
                      %Explorer.Chain.Hash{
                        byte_count: 20,
                        bytes:
                          <<89, 47, 120, 202, 98, 102, 132, 20, 109, 56, 18, 133, 202, 0, 221,
                            145, 179, 117, 253, 17>>
                      } = second_address_hash,
                      %Explorer.Chain.Hash{
                        byte_count: 20,
                        bytes:
                          <<187, 123, 130, 135, 243, 240, 169, 51, 71, 74, 121, 234, 228, 44, 188,
                            169, 119, 121, 17, 113>>
                      } = third_address_hash,
                      %Explorer.Chain.Hash{
                        byte_count: 20,
                        bytes:
                          <<210, 193, 91, 230, 52, 135, 86, 246, 145, 187, 152, 246, 13, 254, 190,
                            97, 230, 190, 59, 86>>
                      } = fourth_address_hash,
                      %Explorer.Chain.Hash{
                        byte_count: 20,
                        bytes:
                          <<221, 47, 30, 110, 73, 130, 2, 232, 109, 143, 84, 66, 175, 89, 101,
                            128, 164, 240, 60, 44>>
                      } = fifth_address_hash
                    ],
                    blocks: [
                      %Explorer.Chain.Hash{
                        byte_count: 32,
                        bytes:
                          <<209, 52, 30, 145, 228, 166, 153, 192, 47, 187, 24, 4, 84, 20, 80, 18,
                            144, 134, 68, 198, 200, 119, 77, 16, 251, 182, 96, 253, 27, 146, 104,
                            176>>
                      }
                    ],
                    logs: [],
                    transactions: [
                      %Transaction{
                        ...
                        hash: %Explorer.Chain.Hash{
                          byte_count: 32,
                          bytes:
                            <<76, 188, 236, 37, 153, 153, 224, 115, 252, 79, 176, 224, 228, 166,
                              18, 66, 94, 61, 115, 57, 47, 162, 37, 255, 36, 96, 161, 238, 171,
                              66, 99, 10>>
                        }
                      },
                      %Transaction{
                        ...
                        hash: %Explorer.Chain.Hash{
                          byte_count: 32,
                          bytes:
                            <<240, 237, 34, 44, 16, 174, 248, 135, 4, 196, 15, 198, 34, 220, 218,
                              174, 13, 208, 242, 122, 154, 143, 4, 28, 171, 95, 190, 255, 254,
                              174, 75, 182>>
                        }
                      }
                    ]
          ...
          assert first_address.fetched_coin_balance == %Wei{
                   value: Decimal.new(1_999_953_415_287_753_599_000)
                 }
          ...
          assert second_address.fetched_coin_balance == %Wei{
                   value: Decimal.new(50_000_000_000_000_000)
                 }
          ...
          assert third_address.fetched_coin_balance == %Wei{
                   value: Decimal.new(30_827_986_037_499_360_709_544)
                 }
          ...
          assert fourth_address.fetched_coin_balance == %Wei{
                   value: Decimal.new(500_000_000_001_437_727_304)
                 }
          ...
          assert fifth_address.fetched_coin_balance == %Wei{
                   value: Decimal.new(930_417_572_224_879_702_000)
                 }
        ...
        EthereumJSONRPC.Nethermind ->
                        ...
                        %Address{
                          hash:
                            %Explorer.Chain.Hash{
                              byte_count: 20,
                              bytes:
                                <<83, 2, 193, 55, 89, 18, 245, 106, 120, 225, 88, 2, 243, 12, 105,
                                  60, 78, 174, 128, 181>>
                        ...
                        %Address{
                          hash:
                            %Explorer.Chain.Hash{
                              byte_count: 20,
                              bytes:
                                <<139, 243, 141, 71, 100, 146, 144, 100, 242, 212, 211, 165, 101,
                                  32, 167, 106, 179, 223, 65, 91>>
                            } = first_address_hash
                        },
                        %Address{
                          hash:
                            %Explorer.Chain.Hash{
                              byte_count: 20,
                              bytes:
                                <<232, 221, 197, 199, 162, 210, 240, 215, 169, 121, 132, 89, 192,
                                  16, 79, 223, 94, 152, 122, 202>>
                            } = second_address_hash
                        }
                      ],
                      blocks: [
                        %Chain.Block{
                          hash: %Explorer.Chain.Hash{
                            byte_count: 32,
                            bytes:
                              <<246, 180, 184, 200, 141, 243, 235, 210, 82, 236, 71, 99, 40, 51,
                                77, 192, 38, 207, 102, 96, 106, 132, 251, 118, 155, 61, 60, 188,
                                204, 132, 113, 189>>
                          } = block_hash
                        }
                      ],
                      logs: [
                        %Log{
                          ...
                          transaction_hash: %Explorer.Chain.Hash{
                            byte_count: 32,
                            bytes:
                              <<83, 189, 136, 72, 114, 222, 62, 72, 134, 146, 136, 27, 174, 236,
                                38, 46, 123, 149, 35, 77, 57, 101, 36, 140, 57, 254, 153, 47, 255,
                                212, 51, 229>>
                          }
                        }
                      ],
          ...
          assert second_address.fetched_coin_balance == %Wei{
                   value: Decimal.new(252_460_837_000_000_000_000_000_000)
                 }
             ...
             %{id: id, method: "eth_getBlockByNumber", params: ["0x708677", true]} ->
               %{
                 ...
                 result: %{
                   ...
                   "mixHash" =>
                     "0xda53ae7c2b3c529783d6cdacdb90587fd70eb651c0f04253e8ff17de97844010",
                   "nonce" => "0x0946e5f01fce12bc",
                   "number" => "0x708677",
                   "parentHash" =>
                     "0x62543e836e0ef7edfa9e38f26526092c4be97efdf5ba9e0f53a4b0b7d5bc930a",
                   "receiptsRoot" =>
                     "0xa7d2b82bd8526de11736c18bd5cc8cfe2692106c4364526f3310ad56d78669c4",
                   "sealFields" => [
                     "0xa0da53ae7c2b3c529783d6cdacdb90587fd70eb651c0f04253e8ff17de97844010",
                     "0x880946e5f01fce12bc"
                   ],
                   "sha3Uncles" =>
                     "0x483a8a21a5825ad270f358b3ea56e060bbb8b3082d9a92ec8fa17a5c7e6fc1b6",
                   "size" => "0x544c",
                   "stateRoot" =>
                     "0x85daa9cd528004c1609d4cb3520fd958e85983bb4183124a4a9f7137fd39c691",
                   "timestamp" => "0x5c8bc76e",
                   "totalDifficulty" => "0x201a42c35142ae94458",
                   "transactions" => [],
                   "transactionsRoot" =>
                     "0xcd6c12fa43cd4e92ad5c0bf232b30488bbcbfe273c5b4af0366fced0767d54db",
                   "uncles" => []
                 }
               }
